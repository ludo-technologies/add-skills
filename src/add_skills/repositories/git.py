...
def clone_repo(source: SkillSource, target_dir: Path | None = None) -> Path:
    ...
    Raises:
        GitError: If source is not a remote repository or clone fails.
    """
    if source.clone_url is None:
        raise GitError("Cannot clone a local source")
    ...
    if source.subpath:
        ...
        if not subpath.exists():
            raise GitError(f"Subpath not found in repository: {source.subpath}")
        return subpath
